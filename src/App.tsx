...
export default function App() {
  ...
  // 핵심 상태 변수 (TS2304 에러 방지를 위해 반드시 선언)
  ...
  const startExam = (exam: Exam) => {
    ...
    setCurrentExamId(exam.id); // currentExamId 상태 업데이트
    if (exam.mode === 'test') {
      setTestAnswers({});
    } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      <nav className="p-4 bg-white border-b flex justify-between items-center">
        <h1 className="text-blue-600 font-bold cursor-pointer" onClick={() => setView('home')}>WÜRTH QUIZ</h1>
        {userProfile && (
      ...
      <main className="p-6 max-w-4xl mx-auto">
            ...
            {adminTab === 'bank' && (
              <div className="space-y-4">
                ...
                <div className="grid gap-2">
                  {filteredBank.map(q => (
                    <label key={q.id} className="bg-white p-4 rounded-xl border flex gap-3 cursor-pointer">
                      ...
                      <div><span className="text-[10px] text-blue-500 font-bold">{q.category}</span><p className="font-bold text-sm">{q.text}</p></div>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="grid gap-2">
                {exams.map(ex => (
                  <div key={ex.id} className="bg-white p-4 rounded-xl border flex justify-between items-center shadow-sm">
                    ...
                    <button onClick={() => startExam(ex)} className="text-[10px] bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg font-bold">시작하기</button>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            {exams.find(e => e.id === currentExamId)?.mode === 'test' && questionQueue.length === 0 && (
              <div className="space-y-4">
                ...
                <button onClick={() => { setStudentScore(100); setView('student-result'); }} className="w-full bg-blue-700 text-white py-4 rounded-2xl font-bold">평가 제출</button>
              </div>
            )}
          </div>
        )}
        ...
        {view === 'student-result' && (
          <div className="py-20 text-center space-y-4">
            <h2 className="text-3xl font-black">수고하셨습니다!</h2>
